#!/usr/bin/env sh
# SessionStart hook: injects the full adderall ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.adderall-always (default ~/.claude).
...
flag_path="$claude_dir/.adderall-always"
...
skill_path="$script_dir/../skills/adderall/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'FOCUS MODE ACTIVE (always-on). The ruleset below applies to every response. "stop adderall" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
